// This file is copied from https://github.com/pytorch/pytorch/blob/4a60bdc6b48caf621407de0ba28f2420024eab32/aten/src/ATen/native/GroupedMMUtils.h
...
std::optional<c10::ScalarType> out_dtype) {
  ...
  if (bias.has_value()) {
    ...
    TORCH_CHECK(bias_tensor.dtype() == at::kBFloat16,
                "Bias must be BFloat16, got ", bias_tensor.scalar_type());
    TORCH_CHECK(bias_tensor.dim() == 2,
                "Bias must be 2D [group_count, N] for per-group bias, got ",
                bias_tensor.dim(), "D");

    // Determine expected dimensions
    int64_t expected_N = (mat_b.dim() == 3) ? mat_b.size(2) : mat_b.size(1);
    int64_t expected_groups = (mat_a.dim() == 3) ? mat_a.size(0) :
                              (mat_b.dim() == 3) ? mat_b.size(0) :
                              (offs.has_value() ? offs->size(0) : 1);

    TORCH_CHECK(bias_tensor.size(0) == expected_groups,
                "Bias first dimension must match group_count, expected ",
                expected_groups, " got ", bias_tensor.size(0));
    TORCH_CHECK(bias_tensor.size(1) == expected_N,
                "Bias second dimension must match N dimension, expected ",
                expected_N, " got ", bias_tensor.size(1));
    TORCH_CHECK(bias_tensor.is_contiguous(),
                "Bias must be contiguous");
    TORCH_CHECK(uint64_t(bias_tensor.data_ptr()) % 16 == 0,
                "Bias data pointer must be 16-byte aligned for TMA transfers, got address ",
                bias_tensor.data_ptr());
    int alignment = 16 / bias_tensor.element_size();
    TORCH_CHECK(bias_tensor.size(1) % alignment == 0,
                "Bias N dimension must be multiple of ", alignment,
                " elements (16 bytes) for TMA alignment, got ", bias_tensor.size(1),
                ". Typical MoE hidden dimensions (128, 256, 512, 1024, 2048, 4096) satisfy this.");
  }
}
