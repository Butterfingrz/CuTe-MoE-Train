// This file is adapted from https://github.com/pytorch/pytorch/blob/4a60bdc6b48caf621407de0ba28f2420024eab32/aten/src/ATen/native/cuda/GroupMMCommon.cuh
...
__global__ void prepare_grouped_gemm_data(
  ...
  // Populate bias pointer array using regular pattern (like B_ptrs)
  // Bias shape: [num_experts, N] - 2D regular tensor
  // All experts have fixed N dimension (output_dim)
  // Uses actual stride(0) from tensor_StrideBias, not assumed shape[1]
  if (bias_ptrs != nullptr && bias_base != nullptr) {
    bias_ptrs[tid] = bias_base + tid * tensor_StrideBias[0];
  }
...
// Populate per-group pointer array for swiglu output tensor.
// The swiglu output has shape [total_M, N_out] where N_out = N_full / 2.
// Each group's pointer = swiglu_base + cumulative_M_before_group * stride_swiglu.
// Only used in ragged-M (M<0) path, which is the GPT-OSS MoE path.
template <typename DtypeOutput>
__global__ void prepare_swiglu_ptrs(
    DtypeOutput* swiglu_base,
    DtypeOutput** swiglu_ptrs,
    const int32_t* offs,
    int32_t stride_swiglu) {
  int32_t tid = threadIdx.x;
  if (swiglu_base == nullptr || swiglu_ptrs == nullptr) return;
  swiglu_ptrs[tid] = tid == 0
      ? swiglu_base
      : swiglu_base + offs[tid - 1] * stride_swiglu;
}
